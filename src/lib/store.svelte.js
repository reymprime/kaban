...
export const vault = $state({
  ...
  appLock: { configured: false, locked: false }, // 4-digit PIN gate on app open
...
function applyThemeColor(dark) {
  ...
  else if (vault.palette === 'crimson') color = dark ? '#200707' : '#F6F7F9';
...
export async function setupAppPin(pin) {
  const salt = makeSalt();
  const key = await deriveKey(pin, salt);
  const verifier = await encryptText(key, 'kaban-pin-ok');
  const meta = { key: 'applock', salt, verifier, createdAt: Date.now() };
  await db.putMeta(meta);
  appLockMeta = meta;
  vault.appLock.configured = true;
  vault.appLock.locked = false;
}

export async function verifyAppPin(pin) {
  if (!appLockMeta) return false;
  try {
    const key = await deriveKey(pin, appLockMeta.salt);
    return (await decryptText(key, appLockMeta.verifier)) === 'kaban-pin-ok';
  } catch {
    return false;
  }
}
...
export async function unlockApp(pin) {
  ...
  if (ok) vault.appLock.locked = false;
  return ok;
}

export async function changeAppPin(oldPin, newPin) {
  const ok = await verifyAppPin(oldPin);
  if (!ok) throw new Error('Current PIN is incorrect');
  await setupAppPin(newPin); // overwrites the stored verifier
}

export async function disableAppPin(pin) {
  const ok = await verifyAppPin(pin);
  if (!ok) throw new Error('PIN is incorrect');
  await db.removeMeta('applock');
  appLockMeta = null;
  vault.appLock.configured = false;
  vault.appLock.locked = false;
}

// Re-arm the lock (called on app open and when returning from the background).
export function lockApp() {
  if (vault.appLock.configured) vault.appLock.locked = true;
}
